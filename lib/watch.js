...
function respawn(options) {
  var argv = process.argv;
  var watchReady = true;
  var param = argv.slice(1);
  // 调试子进程使用
  // param.unshift('--inspect-brk=5858');
  // 更改：增加color，保留色彩输出
  var child = child_process.spawn(argv[0], param.concat('--child-flag').concat('--color'));
  ...
  child.stdout.on('data', function(data) {
    ...
    // 更改：监听子进程 watch ready了
    if (~data.toString('utf-8').indexOf('finish a child build') && watchReady) {
      watchReady = false;
      options.watchReady && options.watchReady();
      return;
    }
...
function watch(options, next) {
  ...
  // 注释：主进程去创建子进程 做watch功能
  if (isMaster()) {
    (function() {
      var damen = arguments.callee;
      var child = respawn(options);
      child.on('exit', function(code) {
        !code || damen();
      });
    })();
    options.afterCompiled && options.afterCompiled('run a child process');
    return
  }
